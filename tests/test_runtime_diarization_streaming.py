...
from runtime.diarization.streaming import DiarizationStreamState


def test_diarization_stream_state_emits_overlapped_chunks():
    config = DiarizationStreamConfig(sample_rate=10, chunk_seconds=1.0, overlap_seconds=0.2)
    state = DiarizationStreamState(config)
...
def test_diarization_stream_flush_returns_remainder_and_resets_buffer():
    config = DiarizationStreamConfig(sample_rate=10, chunk_seconds=2.0, overlap_seconds=0.5, min_flush_seconds=0.2)
    state = DiarizationStreamState(config)
