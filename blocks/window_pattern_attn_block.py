...
from tensor.masking import window_pattern_from_spans, broadcast_mask
...
class WindowPatternAttentionBlock(CausalSelfAttentionBlockBase):
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor | None = None, cache=None) -> torch.Tensor:
        B, T, _ = x.shape
        pat = window_pattern_from_spans(self.spans, T).to(device=x.device, dtype=torch.bool)
        base = broadcast_mask(batch_size=B, num_heads=self.cfg.n_heads, tgt_len=T, src_len=T, causal_mask=pat)
        if mask is not None:
            base = base | mask
