...
from tensor.masking import build_prefix_lm_mask, broadcast_mask
...
class PrefixLMBlock(CausalSelfAttentionBlockBase):
    def __init__(self, cfg: ModelConfig, prefix_len: int, block_cfg: BlockConfig | None = None, drop_path: float = 0.0):
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor | None = None, cache=None) -> torch.Tensor:
        B, T, _ = x.shape
        pm = build_prefix_lm_mask(T, self.prefix_len, device=x.device, dtype=torch.bool)
        base = broadcast_mask(batch_size=B, num_heads=self.cfg.n_heads, tgt_len=T, src_len=T, causal_mask=pm)
        if mask is not None:
            base = base | mask
