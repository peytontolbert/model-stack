...
from tensor.masking import build_dilated_causal_mask, broadcast_mask
...
class DilatedLocalAttentionBlock(CausalSelfAttentionBlockBase):
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor | None = None, cache=None) -> torch.Tensor:
        B, T, _ = x.shape
        dil = build_dilated_causal_mask(T, self.window, self.dilation, device=x.device, dtype=torch.bool)
        base = broadcast_mask(batch_size=B, num_heads=self.cfg.n_heads, tgt_len=T, src_len=T, causal_mask=dil)
        if mask is not None:
            base = base | mask
