...
from tensor.masking import build_banded_mask, broadcast_mask
...
class BandedAttentionBlock(CausalSelfAttentionBlockBase):
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor | None = None, cache=None) -> torch.Tensor:
        B, T, _ = x.shape
        band = build_banded_mask(T, self.bandwidth, device=x.device, dtype=torch.bool)
        attn_mask = broadcast_mask(batch_size=B, num_heads=self.cfg.n_heads, tgt_len=T, src_len=T, causal_mask=band)
        if mask is not None:
