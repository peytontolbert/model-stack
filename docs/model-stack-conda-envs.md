...
| --- | --- | --- | --- | --- |
| `black-forest-labs--FLUX.2-dev` | `diffusers_cuda_bridge` | `ai` | Component schema verified, but generic pipeline-level `device_map=balanced` is flawed for this snapshot: the whole ~60GiB BF16 transformer is assigned to CPU. Use a custom Flux2 bridge with component-level transformer submodule placement and cached prompt embeddings; do not use the text encoder on the hot path. | `reports/world-model-smokes/black-forest-labs--FLUX.2-dev.pipeline_flaw.ai.json` |
...
| `black-forest-labs--FLUX.2-klein-9B` | `diffusers_cuda_bridge` | `ai` candidate | Full placement and 256x256 1-step latent generation verified with `device_map=balanced` and explicit `max_memory`; warmed load 5.46s, generation 5.03s. | `reports/world-model-smokes/black-forest-labs--FLUX.2-klein-9B.flux2_full_256x256_1step.latent.ai.json` |
