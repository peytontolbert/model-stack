  ...
  for (int64_t k0 = slice_begin; k0 < slice_end; k0 += TileK) {
    ...
    for (int index = thread_linear; index < TileCols * PackedTileK; index += Threads) {
      ...
      for (int offset = 0; offset < 4; ++offset) {
        ...
        if (tile_k < TileK) {
          w_tile[tile_col][tile_k] =
              (global_col < layout.logical_out_features && (global_k0 + offset) < slice_end &&
               (global_k0 + offset) < layout.logical_in_features)
                  ? static_cast<int8_t>(DecodeSignedTernaryCode(packed_value, global_k0 + offset))
                  : static_cast<int8_t>(0);
        }
      }
    ...
    if (row < rows) {
      ...
      for (int idx = 0; idx < ColsPerThread; ++idx) {
        ...
        #pragma unroll
        for (int kk = 0; kk < TileK; ++kk) {
          if (k0 + kk >= slice_end || k0 + kk >= layout.logical_in_features) {
            break;
          }
          acc += static_cast<float>(x_tile[local_row][kk]) * static_cast<float>(w_tile[tile_col][kk]);
        }
...
__global__ void bitnet_linear_splitk_finalize_kernel(
  ...
  float value = workspace[idx] * ResolveRowScaleDevice(col, scale_values, segment_offsets, layout);
  if (bias != nullptr) {
