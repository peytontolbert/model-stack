...
torch::Tensor CudaEmbeddingForward(
  ...
  if (indices_contig.numel() == 1) {
    const auto token = indices_contig.reshape({-1})[0].item<int64_t>();
    TORCH_CHECK(token >= 0, "CudaEmbeddingForward: indices must be non-negative");
    TORCH_CHECK(token < weight_contig.size(0), "CudaEmbeddingForward: index out of range");
  } else {
