...
from runtime.blocks import apply_native_norm, execute_block_stack, execute_decoder_stack, execute_encoder_stack
...
class TransformerStack(nn.Module):
    ...
    def forward(self, x: torch.Tensor, attn_mask: torch.Tensor | None = None, cache=None) -> torch.Tensor:
        ...
        if self.ln_f is not None:
            x = apply_native_norm(x, self.ln_f)
        return x
...
class EncoderDecoderStack(nn.Module):
    ...
    def forward(
        ...
        if self.ln_f is not None:
            x_dec = apply_native_norm(x_dec, self.ln_f)
        return x_dec
