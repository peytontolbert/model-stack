...
def pack_context_heads(repo_root: str, files: List[str], tok, budget_tokens: int) -> str:
    g = CodeGraph.load_or_build(repo_root)
    lines_out: List[str] = ["Repository snippets:"]
    used = 0
    for rel in files:
...
def pack_context_windows(repo_root: str, files: List[str], tok, budget_tokens: int) -> str:
    g = CodeGraph.load_or_build(repo_root)
    lines_out: List[str] = ["Repository windows:"]
    used = 0
    for rel in files:
        abs_fp = rel if os.path.isabs(rel) else os.path.abspath(os.path.join(g.root, rel))
        try:
...
def collect_function_windows(repo_root: str, files_: List[str], lines_each: int, *, max_candidates: int = 24) -> List[Tuple[str, int, int, int, List[str]]]:
    gg = CodeGraph.load_or_build(repo_root)
    out: List[Tuple[str, int, int, int, List[str]]] = []
    for rel in files_:
        abs_fp = rel if os.path.isabs(rel) else os.path.abspath(os.path.join(gg.root, rel))
        try:
            src_lines = open(abs_fp, "r", encoding="utf-8", errors="ignore").read().splitlines()
        except Exception:
            continue
